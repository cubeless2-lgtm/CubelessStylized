...
- CubelessStylized intentionally keeps the editor-only `UnrealMCP` plugin enabled in `StylizedCubeless.uproject` as this workstation's authoring bridge. Do not report that existing `.uproject` entry as a review issue by itself; still report new C++/Blueprint/PCG/asset/runtime hard references to UnrealMCP.
