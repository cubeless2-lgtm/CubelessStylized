"""Check Git paths for StackOBot animation study commits.

This helper is intentionally local/read-only. It inspects the Cubeless Git
index, or a supplied revision range, and fails if paths outside the StackOBot
animation documentation/tooling scope are present. It is meant to keep unrelated
Unreal assets and PCG work out of StackOBot animation study commits and pushes.
"""
...
def _classify_path(path: str) -> tuple[str, str]:
    if path in ALLOWED_EXACT:
        return "allowed_stackobot_animation_scope", "Exact StackOBot animation tooling, hook, or work-log path."
...
def run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    check_range = str(args.commit_range or "").strip()
    if check_range:
        mode = "range"
        scope_command = ["git", "diff", "--name-status", check_range]
        diff_check_command = ["git", "diff", "--check", check_range]
    else:
        mode = "staged"
        scope_command = ["git", "diff", "--cached", "--name-status"]
        diff_check_command = ["git", "diff", "--cached", "--check"]

    scope_result = _run_command(scope_command, PROJECT_ROOT)
    diff_check = _run_command(diff_check_command, PROJECT_ROOT)

    entries = _parse_name_status(scope_result.get("stdout", "")) if scope_result.get("success") else []
    ...
    report = {
        ...
        "staged_command": scope_result,
        "range_command": scope_result if check_range else None,
...
def _format_summary(report: dict[str, Any]) -> str:
    ...
    lines = [
        ...
        (
            f"schema={report['schema']} mode={report['mode']} range={report['range'] or 'none'} "
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        dest="commit_range",
        default="",
        help="Check a Git revision range such as origin/main..HEAD instead of the staged index.",
    )
    parser.add_argument(
        "--write-report",
        action="store_true",
