...
with --dry-run for CI/smoke checks. It only performs operations that are safe
...
It intentionally does not edit source assets, merge emitters, modify Scratch Pad
modules, or bind renderer materials. Those steps remain in blocked_by_api until
the Niagara edit/inspector API is expanded.
"""
...
def target_value_for_module_input(
    ...
    if material_color is not None and is_module_color_input(input_name):
        ...
        if isinstance(current_value, list) and len(current_value) == 3:
            return intent_value("parsed_color", current_value, rgba[:3])
        if isinstance(current_value, list) and len(current_value) == 4:
            return intent_value("parsed_color", current_value, rgba)
        return intent_value("parsed_color", current_value, rgba)
    ...
    if duration is not None and is_module_duration_input(input_name):
        if isinstance(current_value, (int, float)) and not isinstance(current_value, bool):
            return intent_value("parsed_duration", current_value, float(duration))
    ...
    if size_multiplier is not None and is_module_size_input(input_name):
        ...
        if value is not None:
            return intent_value("parsed_size_multiplier", current_value, value)
    ...
    if spawn_multiplier is not None and is_module_spawn_input(input_name):
        ...
        if value is not None:
            return intent_value("parsed_spawn_multiplier", current_value, value)
    ...
    if velocity_multiplier is not None and is_module_velocity_input(input_name):
        ...
        if value is not None:
            return intent_value("parsed_velocity_multiplier", current_value, value)
    ...
    if opacity_multiplier is not None and is_module_opacity_input(input_name):
        ...
        if value is not None:
            return intent_value("parsed_opacity_multiplier", current_value, value)
...
def apply_module_inputs_via_socket(
            ...
            skipped.append(
                {
                    ...
                    "input_name": row["input_name"],
                    "input_type": row["input_type"],
                    "reason": "no_safe_prompt_intent_match_or_missing_rapid_iteration_value",
                }
            )
            continue
    ...
    return {
        ...
        "unapplied_module_input_intent": {
            "color": material_color[0] if material_color else "",
            "duration_seconds": duration,
            "reason": "" if applications else "No existing RapidIteration module input matched safe name/type hints.",
        },
        "source_policy": recipe.get("write_scope", {}).get("source_policy", ""),
    }
