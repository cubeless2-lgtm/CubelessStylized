...
- Exact isolated source-vs-output RigidBody/Trail attribution is covered by `sample_anim_node_pre_post_runtime_pose(mode=isolated_temp_components)`; same-instance PoseWatch attribution is confirmed for Baddy RigidBody and remains an expansion candidate for Bot Trail and broader node classes.
...
- Full pin/link-level transition condition internals are still not expanded through the Python path.
