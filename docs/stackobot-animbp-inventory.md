...
- Use `inspect_anim_instance_runtime_state` for current live PIE/SIE state names, `set_anim_instance_runtime_property_for_probe` for runtime-only property echo checks, and `sample_anim_state_machine_runtime_response` for restored property-case snapshots. The real `ABP_Bot` state-change matrix now exists at `D:/Git/SampleProject/StackOBot/Saved/MCP/AnimStudy/StackOBot_ABP_Bot_RuntimeDriverMatrix.md`; remaining state-machine gaps are transition weights/blend progress and full K2 call topology.
