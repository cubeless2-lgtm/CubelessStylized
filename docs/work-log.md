...
## 2026-06-13 TestMap volume-owned grass block-mask fixture

- Branch: `codex/pcg-volume-grass-productionization`.
- Scope: built a TestMap-only isolated fixture for the broad-grass rule requested after the spline-pool review. Grass is owned by a `PCGVolume`, not overlapping grass spline handles.
- Tooling: added `Tools/Unreal/stage_pcg_testmap_volume_grass_blockmask.py`. It opens `/Game/Cubeless/TestMap`, creates `_MCP_Temp` graph/source Blueprint assets, and spawns review actors without saving the map by default.
- Graph rule: `GetLandscape -> SurfaceSampler -> actor-property GrassMesh/GrassMaterial copy -> road mask distance filter -> block-tag StaticMesh distance filter -> StaticMeshSpawner ByAttribute`.
- Mesh/material rule: the grass spawner reads `GrassMesh` and `GrassMaterial` from the placed source actor into `DynamicMeshPath` and `DynamicMaterialSlot0`; it is not hardcoded as the only editable production route.
- Road mask decision: direct PCG distance against the open 2-point road spline was partial around the route. The passing TestMap route derives a road-clearance StaticMesh volume mask from the spline, enables single-point/bounds output, and filters with `PCGDistanceSettings` using `BOX_BOUNDS`.
- Block rule: the `block` tagged StaticMesh exclusion is PCG-native through `PCGDataFromActorSettings -> PCGDistanceSettings(BOX_BOUNDS) -> PCGAttributeFilteringSettings`; no Python prune is used.
- Final functional report: `Saved/MCP_PCG/pcg_testmap_volume_grass_blockmask_report.json` passed with `grass=3853`, `road_points=2`, `road_clearance_violations=0`, `block_overlap_violations=0`, and `python_prune_applied=false`.
- Screenshot route: `Saved/MCP_PCG/pcg_testmap_volume_grass_blockmask_visual_qa_clean_report.json` passed using active viewport capture only, with no bookmark writes. Clean screenshot: `Saved/MCP_Screenshots/pcg_testmap_volume_grass_blockmask_clean_active_viewport_visual_qa.png`.
- Notion capture: attempted, but the Notion connector required reauthentication, so this local work-log entry is the durable project memory for this pass.
- Residual risk: this is still a `_MCP_Temp` TestMap fixture. Production should replace the temporary road clearance helper with a reusable road-owned mask/output contract or a proper production PCG graph asset before committing real map art changes.

## 2026-06-13 TestMap visible road review layer

- Branch: `codex/pcg-volume-grass-productionization`.
- Scope: added the next TestMap review step after the volume-owned grass/block-mask fixture: a visible road layer aligned to `MCP_TestMap_VolumeGrass_RoadSource.Road_SourceSpline`.
- Tooling: added `Tools/Unreal/stage_pcg_testmap_visible_road_review.py`. It reads the existing 2-point road source spline, creates `_MCP_Temp` road materials, builds visible road review actors, hides helper mask meshes for clean game-view screenshots, and validates grass clearance.
- Result: `Saved/MCP_RoadPCG/CubelessTestMapVisibleRoadReview.json` passed with `spline_point_count=2`, `spline_length=5880cm`, `visual_mode=static_mesh_rect_layers`, `3` visible road layers, `grass=3853`, and `road_clearance_violations=0`.
- Screenshot route: `Saved/MCP_RoadPCG/CubelessTestMapVisibleRoadReview_ScreenshotQA.json` passed using active viewport capture only, with no bookmark writes. Screenshot: `Saved/MCP_Screenshots/pcg_testmap_visible_road_review_clean_active_viewport_visual_qa.png`.
- Implementation note: `ProceduralMeshActor`/`ProceduralMeshComponent` were unavailable in the current editor session, so this pass used three layered flat `StaticMeshActor` rectangles instead of the preferred continuous procedural ribbon.
- Residual risk: this visible road is a placeholder review surface, not final art. It proves road/grass alignment and clearance, but final production still needs a road-owned visual/mask contract with better edge feather, terrain/landscape blend, and non-rectangular spline-following output.

## 2026-06-13 TestMap moved road/block separation correction

- User review found the block test cube visually overlapping the road, so the TestMap fixture road source spline was moved down to `Y=23100` while the block test actor remains in the upper-right test area.
- The road clearance graph path was changed from the unstable StaticMesh road-mask actor read to `PCGGetSplineSettings -> PCGSplineSamplerSettings(reference points) -> PCGDistanceSettings -> PCGAttributeFilteringSettings`, keeping the broad grass volume-owned and actor-property mesh/material rules intact.
- Final functional report: `Saved/MCP_PCG/pcg_testmap_volume_grass_blockmask_report.json` passed with `grass=3903`, `road_clearance_violations=0`, `block_overlap_violations=0`, and `python_prune_applied=false`.
- Visible road report: `Saved/MCP_RoadPCG/CubelessTestMapVisibleRoadReview.json` passed with `visual_mode=static_mesh_plane_rect_layers`, `3` plane road layers, `grass=3901`, and `road_clearance_violations=0`.
- Screenshots: clean road review `Saved/MCP_Screenshots/pcg_testmap_visible_road_moved_clean_active_viewport_visual_qa.png`; helper diagnostic `Saved/MCP_Screenshots/pcg_testmap_visible_road_moved_block_diagnostic_active_viewport_visual_qa.png` shows the block separated from the moved road.

## 2026-06-13 Road-aware clear/refill sampler update

- Updated `Tools/Unreal/validate_pcg_road_aware_forest_clear_refill.py` so the validation graph now uses `PCGGetSplineSettings -> PCGSplineSamplerSettings(reference points) -> PCGDistanceSettings -> PCGAttributeFilteringSettings`, matching the more reliable TestMap road-clearance pattern.
- Report: `Saved/MCP_RoadPCG/CubelessRoadAwareForestClearRefill_Report.json` passed. Phase A generated `216` instances with active-route violations `0`; phase B generated `210` instances with active-route violations `0`.
- Refill proof: after moving from route A to route B, `restored_old_a_count=26`, proving the old road corridor refills when road clearing is owned by PCG regeneration instead of destructive post-processing.
- Cleanup: temporary clear/refill actors `MCP_RoadAwareForestClearRefill_RoadSource` and `MCP_RoadAwareForestClearRefill_PCGVolume` were removed by the validation script. The only dirty map reported was the already-dirty `/Game/Cubeless/TestMap`.

## 2026-06-13 Field volume-owned grass sampler road-clearance pass

- Updated `Tools/Unreal/stage_pcg_field_volume_owned_grass.py` so the Field staging graph also uses `PCGGetSplineSettings -> PCGSplineSamplerSettings(reference points) -> PCGDistanceSettings -> PCGAttributeFilteringSettings` for road clearance instead of direct spline distance.
- The Field staging graph now records `road_reference_point_distance_increment_cm=120` and `road_filter_extra_clearance_cm=220`. The extra clearance is intentionally larger than the TestMap value because the Field grass instance transforms can land a few centimeters inside the validation line after projection/spawn bounds handling.
- Validation command: `python Tools/Unreal/stage_pcg_field_volume_owned_grass.py --road-clearance-cm 1250 --settle-seconds 10 --no-save`.
- Report: `Saved/MCP_PCG/pcg_field_volume_owned_grass_stage_report.json` passed after settled validation with `grass=94602`, `road_clearance_violations=0`, `block_overlap_violations=0`, and no graph edge errors.
- Screenshot QA: `Saved/MCP_PCG/pcg_field_volume_owned_grass_sampler_clean_visual_qa_report.json` passed using active viewport clean game view only; screenshot `Saved/MCP_Screenshots/pcg_field_volume_owned_grass_sampler_clean_active_viewport_visual_qa.png`.
- Superseded residual risk: this pass still used settled Python block-overlap pruning as a fallback. The next work-log entry replaces that with a graph-owned block-mask route and records the remaining production limitation.
- Visual note: the new Field staging output is dense and should be reviewed as a rule/ownership validation pass, not final grass art tuning.

## 2026-06-13 Field volume-owned grass block-mask graph pass

- Updated `Tools/Unreal/stage_pcg_field_volume_owned_grass.py` so Field block exclusion no longer relies on settled Python instance pruning.
- Direct `PCGDataFromActor(block) -> PCGDistanceSettings(BOX_BOUNDS) -> AttributeFilter` was tested first, but Field async regeneration settled to `grass=0`. The passing Field route reads current `block` tagged StaticMesh bounds during staging, creates graph-owned mask points with `PCGCreatePointsSettings`, then filters grass with `PCGDistanceSettings(BOX_BOUNDS) -> PCGAttributeFilteringSettings`.
- Validation command: `python Tools/Unreal/stage_pcg_field_volume_owned_grass.py --road-clearance-cm 1250 --settle-seconds 10 --no-save`.
- Report: `Saved/MCP_PCG/pcg_field_volume_owned_grass_stage_report.json` passed with `block_graph_enabled=true`, `block_filter_mode=block-tag StaticMesh bounds -> PCGCreatePointsSettings mask + PCGDistanceSettings BOX_BOUNDS + AttributeFilter`, `grass=94602`, `road_clearance_violations=0`, `block_overlap_violations=0`, and `settled_python_block_overlap_prune_applied=false`.
- Screenshot QA: `Saved/MCP_PCG/pcg_field_volume_owned_grass_blockmask_clean_visual_qa_report.json` passed using active viewport clean game view only; screenshot `Saved/MCP_Screenshots/pcg_field_volume_owned_grass_blockmask_clean_active_viewport_visual_qa.png`.
- Residual risk: this is graph-owned at generation time, but it bakes the detected block bounds into the temporary graph when staging runs. If production needs live response to arbitrary block actor movement without restaging, revisit direct actor-data support or a reusable road/block mask contract.

## 2026-06-13 Field native road material visual tuning

- Relaunched `UnrealEditor.exe` for `StylizedCubeless.uproject`, confirmed the UnrealMCP bridge on `127.0.0.1:55557`, and opened `/Game/Cubeless/Map/LVL_Cubeless_PCG_Ecosystem_Field`.
- Legacy road strip actors `MCP_RoadSurfaceVisual_Shoulder` and `MCP_RoadSurfaceVisual_Core` still exist in the Field level and can pollute road visual review if shown. They were only checked/temporarily hidden for review; no delete, permanent hide, or map save cleanup was performed.
- Updated `Tools/Unreal/apply_pcg_road_material_final_tuning.py` and the corresponding `RUNTIME_ROAD_MATERIAL_SPECS` checks in `Plugins/CustomTools/Content/Python/ArtScripts/CubelessRoadPCG.py`.
- Road material change: removed the old unlit/emissive road look, rebuilt the three native road materials as `Default Lit`, opaque, very rough, low-specular materials, and added UV-V edge color blending so road edges blend toward muted grass/duff colors without using translucency.
- Rejected attempt: a translucent opacity-feather pass rendered the ground/road review incorrectly blue in the active viewport, so the final candidate stays opaque and uses color blending instead of opacity blending.
- Native smoke after tuning: `Saved/MCP_RoadPCG/CubelessForestRoadNativeGraphLiveSmoke.json` passed with `status=ready`, `spline_mesh_component_count=288`, `instanced_instance_total=292`, `roadside_clearance_violation_count=0`, and `runtime_material_value_mismatches={}`. The material validator now recognizes `edge_blend_lerp`.
- Screenshot QA: `Saved/MCP_RoadPCG/FieldNativeRoadOpaqueEdgeColorBlend_ScreenshotQA.json` passed using active viewport clean game view only, did not add dirty packages, and did not create/overwrite bookmarks. Screenshot: `Saved/MCP_Screenshots/field_native_road_opaque_edge_color_blend_active_viewport_visual_qa.png`.
- Residual risk: the road is less bright and less yellow than the previous strip, but the current native spline-mesh road still reads as layered plane strips from some views. Full art cleanup likely needs a production road visual structure change, such as irregular width/noise, a real road texture/mask, or landscape/road blend support, not material color tuning alone.
- Notion capture was attempted, but the Notion connector required reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 Dedicated fence runtime split attempt

- Decision: fence use stays separate from `BP_Cubeless_PCG_EcosystemCandidate`. The ecosystem candidate should not be repurposed as a generic fence container; fence gets its own source Blueprint and runtime PCG graph.
- New promotion tooling: added `Tools/Unreal/promote_pcg_fence_runtime.py` to create `/Game/Cubeless/PCG/Runtime/Blueprints/BP_Cubeless_PCG_FenceSourceRuntime` and `/Game/Cubeless/PCG/Runtime/Graphs/PCG_Cubeless_FenceRuntime_Native`.
- Graph intent: consume a tagged open 2-point or longer linear spline source, subdivide it, recreate an open fence spline, and spawn spline mesh fence segments using the source actor `FenceMeshOverride` property.
- Generated asset status: `Content/Cubeless/PCG/Runtime/Graphs/PCG_Cubeless_FenceRuntime_Native.uasset` was created, but the source Blueprint asset was not successfully written to disk in the current editor session.
- Validation status: the latest saved report `Saved/MCP_PCG/CubelessFenceRuntimePromotion_Report.json` still represents the earlier failed `BP_Cubeless_PCG_FenceRuntime` attempt, with `spline_mesh_component_count=0` and a stale `TRASH_PCGComponent_3` component reference.
- Bridge status: the editor process was alive and `127.0.0.1:55557` accepted TCP connections, but even a minimal `execute_python` ping through `Tools/McpBridge/bridge_exec.py` timed out. The promotion script was therefore refactored so future runs schedule heavy work on editor tick instead of blocking the bridge response.
- Next safe step: restart or clear the current UnrealMCP execute queue, then rerun the dedicated fence promotion script and require a fresh passing report before treating the fence runtime split as complete.

## 2026-06-13 Dedicated fence runtime split completion

- Supersedes the previous attempt note: the editor was restarted, the UnrealMCP bridge recovered on `127.0.0.1:55557`, and the dedicated fence runtime assets were successfully created.
- Assets: `/Game/Cubeless/PCG/Runtime/Blueprints/BP_Cubeless_PCG_FenceSourceRuntime` and `/Game/Cubeless/PCG/Runtime/Graphs/PCG_Cubeless_FenceRuntime_Native`.
- Source actor rule: the fence source Blueprint owns `Fence_SourceSpline` and exposes `FenceMeshOverride`; the PCG graph reads that actor property and uses it for spline mesh spawning. The descriptor fallback mesh was not used in the passing validation.
- Graph rule: `GetSplineData -> SubdivideSpline -> CreateSpline(open) -> GetActorProperty(FenceMeshOverride) -> CopyAttributes -> SpawnSplineMesh`. This keeps open 2-point splines valid for fence/guide/linear placement and keeps fence separate from `BP_Cubeless_PCG_EcosystemCandidate`.
- UnrealMCP bridge fix: `Plugins/UnrealMCP/Source/UnrealMCP/Private/UnrealMCPBridge.cpp` now dispatches `set_spline_component_points` and `refresh_pcg_components`; `LiveCoding.CompileSync` reported success in the editor.
- Validation route: native `refresh_pcg_components` is the reliable wait/poll path for this PCG graph. The older tick-only Python wait path can leave the report stale because graph attachment/generation may not settle before validation.
- Final report: `Saved/MCP_PCG/CubelessFenceRuntimePromotion_Report.json` passed in `socket_orchestrated_native_refresh` mode with `managed_resource_count=19`, `generated_output_data_count=1`, `spline_mesh_component_count=19`, `actor_property_mesh_override_pass=true`, and `descriptor_fallback_used=false`.
- Dirty map note: the validation actors live in the already-dirty Field level during review; no level save was attempted.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate roadside falloff preset

- Decision applied: `BP_Cubeless_PCG_EcosystemCandidate` is no longer treated as a fence/generic pool path for new work. Fence owns the separate dedicated runtime path; EcosystemCandidate now has a new roadside tree+grass falloff preset.
- Added production-candidate graph asset: `/Game/Cubeless/PCG/ProductionCandidates/Graphs/PCG_Cubeless_EcosystemCandidate_RoadsideFalloff`.
- Updated `Plugins/CustomTools/Content/Python/ArtScripts/CubelessEDPCG.py` with preset `6` / `RoadsideSplineFalloff`. The preset applies the new graph through `PCG_Style`, leaves `PCG_Tree` and material preview inactive for generation, and keeps existing presets `1` through `5` unchanged.
- Graph rule: sample the Candidate actor's own open spline, duplicate side bands at increasing offsets, and use wider sampling intervals farther from the spline so density falls off by distance. Meshes are read from BP actor properties `GrassMeshOverride` and `TreeMeshOverride`.
- Validation tool: added `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py`. It stages one unsaved TestMap validation actor, applies the real EDPCG preset-6 path, waits through native `refresh_pcg_components`, and writes a report without saving the level.
- Functional report: `Saved/MCP_PCG/CubelessRoadsideEcosystemCandidateFalloff_Report.json` passed with grass band counts `near=76`, `mid=40`, `far=15`; tree band counts `inner=20`, `outer=12`; `mesh_override_pass=true`; `clearance_pass=true`; and `spline_point_count=2`.
- Screenshot QA: `Saved/MCP_PCG/pcg_roadside_ecosystem_candidate_falloff_visual_qa_report.json` passed using active viewport capture only. Screenshot: `Saved/MCP_Screenshots/pcg_roadside_ecosystem_candidate_falloff_active_viewport_visual_qa.png`.
- Dirty map note: the TestMap validation actor is unsaved review state; no level save was attempted.
- Residual risk: this first roadside preset validates density falloff and actor-property mesh overrides, not final art composition. Material override variables for this specific graph were not added yet; if art review wants material-level variation, add explicit `GrassMaterialOverride`/`TreeMaterialOverride` actor properties in a separate pass.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate dense grass surface fix

- User review found the selected roadside ecosystem PCG was the correct direction, but grass needed to be much denser and was appearing under the visible surface because the graph projected to a Landscape height below the TestMap review surface.
- Updated `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py` and regenerated `/Game/Cubeless/PCG/ProductionCandidates/Graphs/PCG_Cubeless_EcosystemCandidate_RoadsideFalloff`.
- Graph change: removed the fixed `Get Landscape -> Project` branch for this candidate and made the roadside graph follow the authored open guide spline's world height. This keeps the graph suitable for road/guide splines placed on the intended surface and avoids dropping grass to the wrong Landscape actor height during TestMap review.
- Density/art change: increased grass sampling density to `790` instances, kept distance falloff (`near=497`, `mid=245`, `far=48`), added `XY` jitter in `PCGTransformPointsSettings` to reduce straight strip rows, and left tree count lower at `16` so the pass remains grass-heavy.
- Material/property rule: added `GrassMaterialOverride` to `BP_Cubeless_PCG_EcosystemCandidate`, exposed it as an editable actor property, and wired grass spawners through `DynamicMaterialSlot0` while keeping mesh selection driven by `GrassMeshOverride` and `TreeMeshOverride`.
- Functional report: `Saved/MCP_PCG/CubelessRoadsideEcosystemCandidateFalloff_Report.json` passed with `clearance_pass=true`, `mesh_override_pass=true`, `material_override_pass=true`, `dense_grass_pass=true`, and `surface_height_pass=true`; grass and tree instance Z ranges were both `2.0`.
- Screenshot QA: `Saved/MCP_PCG/pcg_roadside_ecosystem_candidate_dense_grass_surface_jittered_visual_qa_report.json` passed using active viewport clean game view only, no bookmark creation/overwrite. Screenshot: `Saved/MCP_Screenshots/pcg_roadside_ecosystem_candidate_dense_grass_surface_jittered_active_viewport_visual_qa.png`.
- Residual risk: the current graph is denser and no longer below the surface, but it still approximates a roadside volume with jittered side bands. A future production art pass should replace this with a true area/volume scatter contract if the row structure remains visible in the final level.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate spline ecosystem falloff with rock and external road cut

- User direction changed preset 6 away from a road-only roadside PCG. The production candidate should now be a general open-spline ecosystem ribbon: grass, trees, and rocks are densest near the spline center and fade outward; roads should be cut by a separate external spline.
- Updated `Plugins/CustomTools/Content/Python/ArtScripts/CubelessEDPCG.py`: preset `6` is now labeled `SplineEcosystemFalloff` and loads `/Game/Cubeless/PCG/ProductionCandidates/Graphs/PCG_Cubeless_EcosystemCandidate_SplineEcosystemFalloff`.
- Updated `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py` to generate the new graph, keep spawnable meshes actor-property driven, and validate the new role. `GrassMeshOverride`, `TreeMeshOverride`, and `RockMeshOverride` are all read from the BP actor; `GrassMaterialOverride` remains actor-property driven for grass.
- Added `EnableExternalRoadClearance` to `BP_Cubeless_PCG_EcosystemCandidate` with default `false`. When enabled, the graph reads a separate tagged spline component `CubelessExternalRoadClearanceSpline` and filters output with `PCGDistanceSettings -> PCGAttributeFilteringSettings`. This keeps the ecosystem graph usable without being road-specific.
- Validation uses an unsaved TestMap ecosystem actor plus a separate unsaved `BP_Cubeless_PCG_ForestRoadRuntime` road-clearance actor. The road actor's PCG components are deactivated; only its `Road_SourceSpline` is used as external clearance input.
- Functional report: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed with `grass=681`, `tree=34`, `rock=71`, `external_road_clearance_violations=0`, `mesh_override_pass=true`, `material_override_pass=true`, `surface_height_pass=true`, and all grass/tree/rock falloff checks passing.
- Screenshot QA: `Saved/MCP_PCG/pcg_spline_ecosystem_falloff_rock_roadcut_final_visual_qa_report.json` passed using active viewport clean game view only, no bookmark creation/overwrite. Screenshot: `Saved/MCP_Screenshots/pcg_spline_ecosystem_falloff_rock_roadcut_final_active_viewport_visual_qa.png`.
- Cleanup note: the prior temporary `MCP_Cubeless_PCG_RoadsideEcosystemCandidate_Validation` actor is now removed by the validation script before staging the new actor. The only dirty map during screenshot QA was the existing `/Game/Cubeless/TestMap`; no level save was attempted.
- Residual risk: the graph is still built from jittered spline bands, not a true filled polygon/volume scatter. If final art needs a more natural blob-shaped ecosystem area, replace the side-band construction with a proper area/volume scatter source.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate per-spawner density/range parameters

- User review accepted the spline ecosystem direction but found the density fade too weak. The graph now exposes per-branch actor properties on `BP_Cubeless_PCG_EcosystemCandidate`: `<Branch>DensitySpacingCm`, `<Branch>SpawnOffsetMin`, and `<Branch>SpawnOffsetMax`.
- Graph wiring: every grass/tree/rock branch reads those actor properties through `PCGGetActorPropertySettings`; `DensitySpacingCm` feeds the sampler `DistanceIncrement` pin, while `SpawnOffsetMin` and `SpawnOffsetMax` feed the transform `OffsetMin` and `OffsetMax` pins. Mesh/material override rules remain BP actor-property driven.
- Default tuning was tightened so the center reads heavier and outer bands fade more strongly: final counts are grass `467/338/67/11` (`center/inner/mid/far`, total `883`), tree `12/10/0/2` (total `24`), and rock `39/26/0/2` (total `67`).
- Functional report: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed with `edge_error_count=0`, `external_road_clearance_violations=0`, `mesh_override_pass=true`, `material_override_pass=true`, `surface_height_pass=true`, and all grass/tree/rock falloff checks passing.
- Screenshot QA: `Saved/MCP_PCG/pcg_spline_ecosystem_stronger_parameterized_fade_visual_qa_report.json` passed using active viewport clean game view only, no bookmark creation/overwrite, and `dirty_package_added_count=0`. Screenshot: `Saved/MCP_Screenshots/pcg_spline_ecosystem_stronger_parameterized_fade_active_viewport_visual_qa.png`.
- C++/API backlog note: this pass did not require C++ source edits, but the repeated BP-variable creation and PCG actor-property pin wiring should be batched later as a native UnrealMCP/PCG graph-authoring helper. See `docs/pcg-cpp-improvement-backlog.md` item `5l`.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate PresetType UI max fix

- User found the `PresetType` control was clamped to max `5`, so preset `6` (`SplineEcosystemFalloff`) could not be selected from the Details panel even though the selector code defined it.
- Added UnrealMCP bridge command `set_blueprint_variable_metadata` in `Plugins/UnrealMCP` and a sibling Python MCP wrapper in `D:\Git\unreal-mcp-cubeless\Python\tools\node_tools.py`. This command updates metadata on existing Blueprint member variables instead of recreating them.
- Applied the command to `/Game/Cubeless/PCG/ProductionCandidates/Blueprints/BP_Cubeless_PCG_EcosystemCandidate.PresetType`, setting `ClampMin=1`, `ClampMax=6`, `UIMin=1`, `UIMax=6`, and a tooltip listing presets `1` through `6`.
- Verification: `LiveCoding.CompileSync` completed, the command returned verified metadata with max `6`, the Blueprint compiled and saved, the selected validation actor changed from `PresetType=5` to `PresetType=6`, and native `refresh_pcg_components` regenerated `PCG_Style` with the `PCG_Cubeless_EcosystemCandidate_SplineEcosystemFalloff` graph.
- Python verification: `python -m py_compile` passed for the sibling MCP wrapper/server files plus the current PCG validation/selector scripts.
- Dirty scope: this intentionally modifies the parent UnrealMCP plugin, the sibling MCP Python layer, and the candidate Blueprint asset. The TestMap validation actor remains unsaved review state; no level save was attempted.

## 2026-06-13 EcosystemCandidate grid-gradient falloff replacement

- User review found the per-branch inner/mid/outer parameter model too complex and the spline start-point trees visually odd. The preset-6 graph has been rebuilt as a grid-driven continuous density gradient instead of stacked spline-sampler branches.
- Graph model: `CreatePointsGrid -> Distance To Source Spline -> DensityRemap(invert distance) -> AttributeNoise spawn roll -> AttributeFilter(spawn roll <= density) -> mesh/material actor-property copy -> optional external road clearance -> StaticMeshSpawner`.
- Exposed controls were reduced to coarse actor properties: `EcosystemGridExtents`, `EcosystemGridCellSize`, `EcosystemWidthCm`, `EcosystemGrassSpawnRatio`, `EcosystemTreeSpawnRatio`, and `EcosystemRockSpawnRatio`. Legacy band variables are hidden from instance editing but not destructively removed.
- Important UE 5.7 API note: `BlueprintEditorLibrary.get_basic_type_by_name("float")` produced an `int` pin type in this editor session. New scalar BP variables for fractional spawn ratios must use `get_basic_type_by_name("real")`. Incorrect `*DensityScale` variables were hidden and replaced by the real-typed `*SpawnRatio` variables.
- Functional report: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed with grass `1059`, tree `55`, rock `104`, `external_road_clearance_violations=0`, `mesh_override_pass=true`, `material_override_pass=true`, `surface_height_pass=true`, and `endpoint_cluster_pass=true`.
- Screenshot QA passed without creating or overwriting bookmarks. Captures: `Saved/MCP_Screenshots/pcg_spline_ecosystem_grid_gradient_active_viewport_visual_qa.png`, `Saved/MCP_Screenshots/pcg_spline_ecosystem_grid_gradient_overview_fixed_active_viewport_visual_qa.png`, and `Saved/MCP_Screenshots/pcg_spline_ecosystem_grid_gradient_start_endpoint_active_viewport_visual_qa.png`.
- Visual review note: the start/end cap clump caused by repeated spline sampler branch first-points is gone. Current art still reads as a linear ecosystem ribbon, with final texture/material polish deferred.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate spline-local row grid correction

- User screenshot review found the grid-gradient replacement still did not follow the edited spline path. Root cause: `CreatePointsGrid` generates a fixed actor-local point domain; the graph used spline distance for density, but the point source itself was not a spline-following ribbon.
- Rebuilt preset-6 graph generation in `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py` as a spline-local row grid: `PCGSplineSampler(OnSpline centerline) -> PCGPointExtentsModifier(row spacing bounds) -> PCGDuplicatePoint(left/right local rows) -> PCGMergeSettings -> PCGDistanceSettings -> DensityRemap -> spawn probability/filter -> actor-property mesh/material copy -> optional external road clearance -> StaticMeshSpawner`.
- The graph now keeps open 2-point splines valid and follows actor rotation/spline direction. Validation actor staging uses a rotated 2-point open spline to catch fixed-world or fixed-grid regressions.
- Exposed instance controls stay coarse: `EcosystemRibbonStepCm`, `EcosystemWidthCm`, `EcosystemGrassSpawnRatio`, `EcosystemTreeSpawnRatio`, and `EcosystemRockSpawnRatio`. Legacy grid, branch, and planar subdivision variables are hidden from instance editing but not destructively removed.
- Functional report: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed with grass `768`, tree `40`, rock `73`, `external_road_clearance_violations=0`, `mesh_override_pass=true`, `material_override_pass=true`, `surface_height_pass=true`, and all grass/tree/rock falloff checks passing.
- Screenshot route: active viewport capture was verified without creating or overwriting bookmarks. Clean perspective report: `Saved/MCP_PCG/pcg_spline_ecosystem_following_rows_camera_fixed_visual_qa_report.json`; screenshot: `Saved/MCP_Screenshots/pcg_spline_ecosystem_following_rows_camera_fixed_active_viewport_visual_qa.png`.
- Top-down overlay capture for spline-following review: `Saved/MCP_PCG/pcg_spline_ecosystem_following_rows_topdown_visual_qa_report.json`; screenshot: `Saved/MCP_Screenshots/pcg_spline_ecosystem_following_rows_topdown_active_viewport_visual_qa.png`. This capture shows the selected spline/actor overlay and the spawned trees/rocks aligned along the centerline.
- Capture API note: an initial viewport camera call using the `unreal.Rotator(...)` constructor produced swapped/incorrect rotation and captured sky only. The reliable route is to assign `Rotator.pitch/yaw/roll` explicitly and call `UnrealEditorSubsystem.set_level_viewport_camera_info`, then capture active viewport.
- Dirty map note: the validation actors live in unsaved `/Game/Cubeless/TestMap` review state; no level save was attempted and the final screenshot captures reported `dirty_package_added_count=0`.
- C++/API backlog note: no C++ was required for the fix, but a future helper should create spline-local ribbon/area point grids from scalar width/row controls and should expose a robust camera-set-and-capture command. See `docs/pcg-cpp-improvement-backlog.md` item `5m`.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate spline edit crash guard

- User reported an editor crash while moving a spline point on preset 6. Latest crash folder `Saved/Crashes/UECC-Windows-FCE501F0429297C988031995254B912B_0003` showed `Assertion failed: IsRotationNormalized()` in `UnrealEditor-PCG`, preceded by repeated `LogPCG: Warning: Invalid metadata key`.
- Root cause assessment: interactive spline movement triggered PCG regeneration while the preset-6 graph was using `PCGDuplicatePoint` in relative space to build local left/right rows. During a drag, transient spline sample transforms can carry non-normalized rotation into downstream PCG transform/spawn code.
- Graph fix: regenerated `/Game/Cubeless/PCG/ProductionCandidates/Graphs/PCG_Cubeless_EcosystemCandidate_SplineEcosystemFalloff` with a `Normalize Ribbon Row Rotation` `PCGTransformPointsSettings` node after `Merge Center And Lateral Rows`. The node preserves row locations but resets merged row point rotation to identity before distance/spawn branches.
- Editing safety fix: preset `6` in `Plugins/CustomTools/Content/Python/ArtScripts/CubelessEDPCG.py` now sets the active `PCG_Style` component to `GenerationTrigger=GenerateOnDemand`, `bRegenerateInEditor=false`, and `bGenerateOnDropWhenTriggerOnDemand=false`. Other production-candidate presets restore normal generate-on-load behavior.
- Validation route fix: `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py` now falls back to `execute_python` when native `refresh_pcg_components` is unavailable in the running bridge. The current crash-focused validation disables external road validation by default because using a heavyweight road/candidate BP as a temporary spline holder can stall editor setup; external road clearance remains part of the graph contract and needs a lighter fixture/API before re-enabling in this route.
- Functional verification: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed at `2026-06-13 17:07:06` with grass `768`, tree `40`, rock `73`, `mesh_override_pass=true`, `material_override_pass=true`, `surface_height_pass=true`, and all falloff checks passing. This run had `external_road_validation_enabled=false`.
- Spline movement smoke: the validation actor's spline point was moved through multiple local positions, then `PCG_Style.generate(true)` was run manually. The component stayed `GenerateOnDemand`, `regenerate_in_editor=false`, manual generate succeeded, and point output stayed `881`.
- Latest log check after the fix found no new `IsRotationNormalized`, `Invalid metadata key`, `Assertion failed`, `Fatal`, or `LogPython: Error` lines, and no new crash folder appeared after `UECC-Windows-FCE501F0429297C988031995254B912B_0003`.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate spline live-link repair

- User review found that after moving the preset-6 spline, the visible generated forest could remain at the previous spline sample location. Root cause: the crash guard intentionally switched `PCG_Style` to `GenerateOnDemand`/`regenerate_in_editor=false`, so native live regeneration no longer ran during spline edits.
- Updated `Plugins/CustomTools/Content/Python/ArtScripts/CubelessEDPCG.py` so preset `6` installs a debounced editor tick watcher for the selected/placed actor. The watcher tracks the tagged spline component signature, waits for edits to settle, then calls `PCG_Style.generate(true)` twice while keeping the component on-demand. This restores editor preview linkage without re-enabling drag-time native PCG regeneration.
- Added stale watcher cleanup behavior: if a validation actor is destroyed and an old callback sees an invalid UObject, it unregisters itself and logs a warning instead of repeatedly emitting `LogPython: Error`.
- Adjusted the preset-6 graph rotation guard in `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py`: the post-merge transform now uses an additive zero rotation rebuild instead of absolute world-identity rotation. This avoids forcing all generated resources into a fixed-world rotation during rotated-spline validation while still exercising the crash guard path.
- Functional verification after re-staging: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed at `2026-06-13 17:48:10` with grass `768`, tree `40`, rock `73`, `mesh_override_pass=true`, `material_override_pass=true`, `surface_height_pass=true`, and all falloff checks passing.
- Spline live-link smoke: moving the validation actor's spline endpoint caused the watcher `generate_count` to increment and output point count to remain `881` without a new editor crash. Direct `cleanup(true) + generate(true)` is not used by the watcher because in this session it preserved graph point output but recreated only partial StaticMeshSpawner resources.
- Residual risk: a bent rotated validation spline can still expose PCG stock-node transform/width-filter limitations. Dynamic `SplineDistance <= EcosystemWidthCm` filtering through `PCGAttributeFilteringSettings` produced zero output from Python graph construction and is recorded in `docs/pcg-cpp-improvement-backlog.md` for a native/helper-level fix.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate spline point add/remove regeneration fix

- User found the critical preset-6 issue: adding or deforming spline points must add/remove/reposition the generated grid points and spawners, but the previous output could stay on the old sampled path.
- Reproduced the failure on `/Game/Cubeless/TestMap`: a 2-point spline generated `447` instances, then adding a third point changed spline length to `15400.0` but plain watcher/manual `generate(true)` left instance count and bounds unchanged. A later 4-point test showed watcher `generate_count` incrementing while output stayed at the prior 3-point result.
- Crash analysis: forcing cache refresh through `cleanup(true)+generate(true)` or same-tick graph reassignment on the old `PCGDuplicatePoint` row graph could trigger repeated `Invalid metadata key` warnings followed by `IsRotationNormalized()` asserts. New crash folders observed during this investigation: `UECC-Windows-61DCE55D4E65A5B4ABD81D90EED6757C_0000`, `UECC-Windows-76C3851F440FE7F401883ABADE2E9E8C_0000`, and `UECC-Windows-C9947A0149E9BB16465F4386F73C50B0_0000`.
- Graph fix: rebuilt `/Game/Cubeless/PCG/ProductionCandidates/Graphs/PCG_Cubeless_EcosystemCandidate_SplineEcosystemFalloff` from duplicate left/right rows to `PCGSplineSampler(OnHorizontal Fill)` for the ribbon point field plus a separate `PCGSplineSampler(OnSpline)` distance reference. This removes relative `PCGDuplicatePoint` transform accumulation from preset 6.
- Selector/watch fix: `CubelessEDPCG.py` now syncs each ecosystem spline point scale to `Scale=(1, EcosystemWidthCm, 1)` for preset 6, includes width/scale in the live signature, reassigns the same graph on spline changes to invalidate stale PCG spline sample cache, then defers `generate(true)` to the next tick.
- Functional verification: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed after the graph rebuild with `grass=680`, `tree=39`, `rock=65`, `mesh_override_pass=true`, `material_override_pass=true`, `surface_height_pass=true`, and all falloff checks passing.
- Dynamic regeneration smoke: 2-point baseline generated `784` total instances; 6-point expansion regenerated automatically to `3756` total (`grass=3232`, `rock=319`, `tree=205`) with bbox max extending to `[26293.8, 31041.0]`; 3-point reduction regenerated automatically to `1550` total (`grass=1340`, `rock=130`, `tree=80`) with no watcher error.
- Screenshot route: active viewport captures only; user bookmarks were not touched. Review captures saved under `Saved/MCP_Screenshots/`: `spline_ecosystem_dynamic_3point_auto_regen_review_v3.png` and `spline_ecosystem_dynamic_3point_auto_regen_topdown.png`. Earlier sky-only attempts showed that Python `unreal.Rotator` constructor order is `(roll, pitch, yaw)` on this route; use explicit pitch/yaw/roll verification before capture.
- Dirty map note: dynamic validation leaves an unsaved TestMap review actor/state (`/Game/Cubeless/TestMap` dirty). No level save or destructive cleanup was attempted.
- C++/API backlog note: updated `docs/pcg-cpp-improvement-backlog.md` item `5m` with the remaining native helper candidate: safe spline-change cache invalidation/regeneration and a width-aware ribbon sampler helper that does not rely on hidden spline scale synchronization.

## 2026-06-13 EcosystemCandidate ribbon width scale visual fix

- User visual review caught an unacceptable preset-6 look: grass meshes were stretched into giant radial strips across the viewport after the `OnHorizontal Fill` graph change.
- Root cause: `EcosystemWidthCm` was synced into spline point `Scale.Y` so the stock spline sampler could create a filled ribbon, but the sampled point transform kept that huge scale. StaticMeshSpawner then applied the ribbon width scale to the spawned meshes.
- Graph fix: inserted `Normalize Ribbon Point Scale` (`PCGTransformPointsSettings`) immediately after `Sample Source Spline Ribbon Area Points`. It preserves sampled locations/rotations, sets point scale to `(1,1,1)`, and only then feeds `Distance To Source Spline` and the spawn branches.
- Functional verification after graph rebuild still passed: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` at `2026-06-13 19:02:58`, with `grass=680`, `tree=39`, `rock=65`, all falloff checks passing, and `mesh_override_pass/material_override_pass/surface_height_pass=true`.
- Scale verification: 2-point baseline instance max scale is now `1.32` instead of inheriting width scale. Dynamic 3-point shrink generated `1550` total instances with max scale `1.319`; dynamic 6-point expansion generated `3756` total instances with max scale `1.32`.
- Live watcher fix: graph reassignment plus one deferred generate could still leave stale point count after the scale-normalized graph rebuild. The watcher now performs two deferred generate passes after graph reassignment. Automatic 3-point shrink and 6-point expansion both passed with `last_error=null`.
- Screenshot evidence: `Saved/MCP_Screenshots/spline_ecosystem_scale_normalized_overview.png` and `Saved/MCP_Screenshots/spline_ecosystem_scale_normalized_6point_overview.png`. These active viewport captures show the stretch bug removed; remaining visual tuning is normal art direction work, not the scale-propagation bug.

## 2026-06-13 EcosystemCandidate spline live-follow cache fix

- User review found that the preset-6 look was acceptable, but moving a spline point could still leave the visible forest on the previous spline path. This was not user misuse: the selected validation actor had the live watcher installed and detected changes, but PCG output stayed stale.
- Root cause: `PCG_Style` is intentionally kept `GenerateOnDemand` to avoid drag-time PCG crashes. The watcher reassigned the graph once, then ran deferred generate calls, but UE's PCG spline sample cache could still keep the old sampled path unless graph reassignment happened again at the actual deferred generate time.
- Selector fix: `CubelessEDPCG.py` now includes actor and spline component transform data in the live signature, debounces spline edits for `0.65s`, then performs `3` deferred generate passes at `0.9s` intervals. Each deferred pass reloads/reassigns the preset-6 graph before `notify_properties_changed_from_blueprint()` and `generate(true)`.
- Verification: after moving the validation spline endpoint from `[34200,38600,0]` to `[25500,43000,0]` without any manual generate, watcher state reached `generate_count=3`, `graph_reassign_count=1`, `deferred_graph_reassign_count=3`, and `last_error=null`. Instance output updated from `5986` total at bbox max `[35438.2,40265.7,0]` to `5802` total at bbox max `[27550.1,43705.3,0]`, proving the generated grass/tree/rock ribbon followed the new spline.
- MCP tooling note: native `set_spline_component_points` was unavailable in the running bridge, and the Python fallback matched a `TRASH_SplineComponent_50` instead of the real `Spline` component when called by component name. The real component was not changed, but this needs a future bridge/tooling fix before using that helper for production spline validation.
- Dirty map note: this validation changed the unsaved TestMap review actor's endpoint only; no level save, bookmark overwrite, or destructive cleanup was attempted.
- Notion capture fallback: the Notion connector still requires reauthentication, so this local work-log entry is the durable project memory for this pass.

## 2026-06-13 EcosystemCandidate RegenerateInEditor test

- User questioned whether the tick/timer watcher is necessary because PCG should already support editor regeneration. Tested the preset-6 validation actor with the watcher explicitly stopped and `PCG_Style` set to `GenerationTrigger=GenerateOnLoad`, `RegenerateInEditor=true`, and `GenerateOnDropWhenTriggerOnDemand=true`.
- Important setup finding: setting those properties on the placed instance component can cause the live component to be renamed to `TRASH_PCGComponent_*` and recreated from the Blueprint component template. For a real test, `PCG_Style_GEN_VARIABLE` on `BP_Cubeless_PCG_EcosystemCandidate` was edited through `SubobjectDataSubsystem`, then the live component was set again. The asset was not saved.
- Regeneration finding: changing actor properties did trigger PCG regeneration with the current spline shape, so `RegenerateInEditor=true` is part of the right production direction. A temporary overgeneration spike to `79,703` instances was traced to bad test actor values (`EcosystemGrassSpawnRatio=205.051516`, `Tree=2.971015`, `Rock=1.787388`, `RibbonStep=55.706125`, `Width=1985`), not to the regenerate flag itself. Resetting to defaults (`RibbonStep=150`, `Width=2800`, `Grass=1.0`, `Tree=0.06`, `Rock=0.10`) returned output to `9,114` total instances on the current long test spline.
- Spline-change finding: direct Python `set_location_at_spline_point()` does not emit the same editor spline-handle edit event as a user drag. With watcher off and `RegenerateInEditor=true`, moving the endpoint from `[34000,46000,0]` to `[30000,50000,0]` did not refresh output by itself; `refresh_pcg_runtime_component()` also left the previous output bounds unchanged. This means automated validation still needs a native helper that performs the real editor spline edit notification, or the user must manually drag in the viewport to confirm the UI path.
- Structure finding: the current graph still reads ribbon width from spline point `Scale.Y`, while the artist-facing control is `EcosystemWidthCm`. Without the watcher, changing the actor width does not automatically synchronize existing spline point scale. Production cleanup should either move width sync into a Blueprint construction/editor hook or remove the hidden spline-scale dependency from the graph.
- Latest log check after the RegenerateInEditor test found no new `IsRotationNormalized`, `Assertion failed`, `Fatal`, or `LogPCG: Error` entries. Older Python probing errors remain in the log from API exploration.

## 2026-06-13 EcosystemCandidate editable spline tangents

- User accepted the `RegenerateInEditor` production direction but found the spline had no tangent handles. Root cause: the validation setup script forced the candidate spline points to `SplinePointType.LINEAR`, so the spline was editable only as straight segments.
- Updated `Plugins/CustomTools/Content/Python/ArtScripts/CubelessEDPCG.py`: preset 6 now keeps `PCG_Style` on `GenerationTrigger=GenerateOnLoad`/`RegenerateInEditor=true` and stops the temporary live watcher instead of reinstalling it. The spline width sync also converts only `LINEAR`/`CONSTANT` points to `CURVE_CUSTOM_TANGENT` and initializes tangents from neighboring points; existing curve/custom tangent edits are preserved.
- Updated `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py`: newly staged validation splines and external road-clearance splines are created with editable `CURVE_CUSTOM_TANGENT` points instead of `LINEAR` points.
- Applied the fix to the current unsaved TestMap validation actor: all 6 `Spline` points now report `SplinePointType.CURVE_CUSTOM_TANGENT` with nonzero arrive/leave tangents. Reapplying preset 6 left `watcher_installed=false`, `PCG_Style.GenerationTrigger=GenerateOnLoad`, and `PCG_Style.RegenerateInEditor=true`.
- Applied and saved the Blueprint template change on `/Game/Cubeless/PCG/ProductionCandidates/Blueprints/BP_Cubeless_PCG_EcosystemCandidate`: `Spline_GEN_VARIABLE` default points are now `CURVE_CUSTOM_TANGENT`.
- Verification: `python -m py_compile` passed for `CubelessEDPCG.py` and `validate_pcg_roadside_ecosystem_falloff.py`. No level save or bookmark overwrite was attempted.

## 2026-06-13 EcosystemCandidate low-density grid-volume repair

- User noted that manual actor parameter edits should be ignored and asked to keep the existing direction without jumping to unsafe high density. The current validation actor values were treated as non-authoritative; the validation script now owns low-density defaults for this pass.
- Root cause assessment: the previous duplicate-row candidate generation path could still hit UE 5.7 PCG transform/metadata instability while editing splines. The graph was rebuilt away from `PCGDuplicatePoint` rows and away from the width-scale `OnHorizontal Fill` route. The current production-candidate graph uses `Create Local Ecosystem Candidate Grid -> Distance To Source Spline -> DensityRemap -> spawn probability/filter -> actor-property mesh/material copy -> optional external road clearance -> StaticMeshSpawner`.
- Parameter cleanup: `EcosystemGridCellSize` is now the exposed candidate-density control. `EcosystemGrassSpacingCm`, `EcosystemRibbonStepCm`, and planar subdivision controls are legacy/hidden for this route. `EcosystemGrassSpawnRatio`, `EcosystemTreeSpawnRatio`, and `EcosystemRockSpawnRatio` remain separate probability controls.
- Safe defaults applied by `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py`: `EcosystemGridCellSize=(240,240,50)`, `EcosystemWidthCm=1400`, `GrassSpawnRatio=0.35`, `TreeSpawnRatio=0.018`, `RockSpawnRatio=0.04`.
- Functional validation passed with `edge_error_count=0`, grass `75`, tree `5`, rock `9`, all falloff checks passing, `mesh_override_pass=true`, `material_override_pass=true`, and `surface_height_pass=true`. The report is `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json`.
- Spline move smoke: after moving the validation spline last point from local `[4200,0,0]` to `[5100,1050,0]` and running manual `generate(true)`, output updated to `82` total instances (`grass=70`, `rock=8`, `tree=4`) with no new PCG assert/crash. This proves the low-density grid candidate still responds to the edited spline distance field when regenerated.
- Screenshots used active viewport capture only and did not touch user bookmarks: `Saved/MCP_Screenshots/spline_ecosystem_grid_low_density_review_v2.png` and `Saved/MCP_Screenshots/spline_ecosystem_grid_low_density_after_spline_move.png`.
- Latest log check found no new `IsRotationNormalized`, `Assertion failed`, `Fatal`, `LogPCG: Error`, or `Invalid metadata key` entries after the repaired graph runs. One `LogPython: Error` remains from an incorrect `SystemLibrary.delay()` probe during the smoke script, followed by successful generate/count verification.
- Dirty map note: `/Game/Cubeless/TestMap` remains dirty from the unsaved validation actor/spline state. No level save, bookmark overwrite, destructive cleanup, or unrelated staging was attempted.
- C++/API backlog note: updated `docs/pcg-cpp-improvement-backlog.md` item `5m` with the remaining helper need: derive candidate grid bounds from spline bounds plus width, perform real editor spline-change notification, and regenerate/invalidate PCG output without relying on Python probes.

## 2026-06-13 EcosystemCandidate spline point add extents fix

- User review found the repaired graph works for the existing shape, but adding spline points could leave the newly added arc empty. This was not user misuse: the candidate point source was an actor-local fixed grid, so new spline sections outside `EcosystemGridExtents` had no candidate points to filter/spawn.
- Updated `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py` and regenerated `/Game/Cubeless/PCG/ProductionCandidates/Graphs/PCG_Cubeless_EcosystemCandidate_SplineEcosystemFalloff` so `EcosystemGridExtents` is exposed on `BP_Cubeless_PCG_EcosystemCandidate` and wired into `PCGCreatePointsGridSettings.GridExtents`. `EcosystemGridCellSize` remains the candidate spacing control.
- Default extents were raised from `(4600,1800,30)` to `(9000,6500,30)`. For the current unsaved 4-point validation spline, the actor was patched to `EcosystemGridExtents=(9117.42,7645.37,30)` based on local spline bounds plus width margin, without overriding the user's density/spawn-ratio edits.
- Current actor regenerate result: `edge_error_count=0`, `generate_error=null`, and `6265` total instances (`grass=6078`, `tree=72`, `rock=115`). The high count is caused by the current actor's user-tuned density values, not by a forced validation default reset.
- Screenshot used active viewport capture only, no bookmark overwrite: `Saved/MCP_Screenshots/spline_ecosystem_grid_extents_after_point_add.png`. The capture shows the added curved spline section now filled.
- Residual risk: this exposes and applies the needed bounds control, but fully automatic artist workflow still needs a Blueprint/editor/native helper that updates `EcosystemGridExtents` whenever spline points are added or moved outside the current grid volume.

## 2026-06-13 Dedicated SplineEcosystemFalloff placement BP

- User decided the preset-6 setup is too error-prone for manual placement. Created a dedicated placement Blueprint: `/Game/Cubeless/PCG/ProductionCandidates/Blueprints/BP_Cubeless_PCG_SplineEcosystemFalloff`.
- The new Blueprint is a duplicate of `BP_Cubeless_PCG_EcosystemCandidate` with sample defaults baked in: `PresetType=6`, `DensityOverride=2`, `TreeOverride=3`, `UseGrassMeshOverride=true`, `UseTreeMeshOverride=true`, `UseRockMeshOverride=true`, `EnableExternalRoadClearance=false`, `EcosystemGridExtents=(9000,6500,30)`, `EcosystemGridCellSize=(240,240,50)`, `EcosystemWidthCm=1400`, `GrassSpawnRatio=0.35`, `TreeSpawnRatio=0.018`, and `RockSpawnRatio=0.04`.
- Default mesh/material overrides are also baked into the new BP: `SM_Grass_Medium01`, `SM_Conifer_05`, `SM_SmallRock_01`, and `MI_Cubeless_PCG_GrassMedium_ForestBalanced`.
- Added `UseGrassMaterialOverride=true` to both the base candidate BP and the dedicated duplicate BP, because the shared spline falloff graph reads that actor property before applying `GrassMaterialOverride`.
- Component template setup: the `Spline` component has the `CubelessPCGProductionCandidateSpline` tag and editable curve tangents; `PCG_Style` has the `PCG_Cubeless_EcosystemCandidate_SplineEcosystemFalloff` graph, `AutoActivate=true`, `GenerationTrigger=GenerateOnLoad`, and `RegenerateInEditor=true`; `PCG_Tree` and `PCG_MaterialPreview` are inactive/on-demand with no graph.
- Smoke validation: spawning `BP_Cubeless_PCG_SplineEcosystemFalloff` in unsaved TestMap without calling the selector produced automatic output after a short settle wait: total `95` instances (`grass=81`, `tree=4`, `rock=10`). Manual `set_graph` was not needed for the successful fresh smoke actor.
- Final smoke validation: `MCP_Cubeless_PCG_SplineEcosystemFalloffBP_FinalSmoke` regenerated through `PCG_Style` with `RegenerateInEditor=true`, produced `97` instances (`grass=83`, `tree=5`, `rock=9`), and added no new `LogPCG: Error`, `LogPython: Error`, assertion, fatal, invalid metadata, or non-normalized rotation log entries after the validation baseline.
- Usage note: artists should place `BP_Cubeless_PCG_SplineEcosystemFalloff` directly for this spline forest/grass/rock falloff role instead of placing `BP_Cubeless_PCG_EcosystemCandidate` and manually matching hidden preset defaults.

## 2026-06-14 SplineEcosystemFalloff overlap pruning

- User rule: grass may visually overlap a little, but should not have full/near-identical self overlap. Tree and rock points should not overlap; tree/tree, rock/rock, and tree/rock overlap are disallowed.
- Updated `Tools/Unreal/validate_pcg_roadside_ecosystem_falloff.py` and regenerated `/Game/Cubeless/PCG/ProductionCandidates/Graphs/PCG_Cubeless_EcosystemCandidate_SplineEcosystemFalloff`.
- Graph change: grass now gets `Bounds Modifier -> Self Pruning(AllEqual)` after jitter/yaw/scale with small validation bounds `(38,38,18)` so partial grass visual overlap remains possible. Tree and rock now use separate random roll seeds, set larger overlap bounds (`tree=(170,170,220)`, `rock=(105,105,60)`), merge together, then run `Self Pruning(LargeToSmall)` before one shared tree/rock `StaticMeshSpawner`. This removes same-category and tree-vs-rock prop overlap, with tree generally winning over rock when their bounds collide.
- Base candidate validation report: `Saved/MCP_PCG/CubelessSplineEcosystemCandidateFalloff_Report.json` passed with total `92` instances (`grass=80`, `tree=4`, `rock=8`), graph `edge_error_count=0`, and overlap violations `0` for grass self, tree self, rock self, and tree-rock cross.
- Dedicated BP smoke: `MCP_Cubeless_PCG_SplineEcosystemFalloffBP_OverlapSmoke` produced total `114` instances (`grass=96`, `tree=8`, `rock=10`) after settle/regeneration. Overlap violations were `0` for grass self, tree self, rock self, and tree-rock cross.
- Log check: after the validation baseline, no new `LogPCG: Error`, `LogPython: Error`, assertion, fatal, invalid metadata, or non-normalized rotation log entries were found.

## 2026-06-14 PCG dungeon C++/API preflight

- User wants the PCG dungeon plan to follow the Aardman-style PCG + Geometry Script indoor workflow, with all modular meshes generated by Geometry Script rather than hand-authored mesh imports.
- Preflight result: the MVP does not require new project C++ or new UnrealMCP API. `StylizedCubeless.uproject` already enables `PCG`, `PCGGeometryScriptInterop`, `PCGPythonInterop`, and related PCG interop plugins. `PCGGeometryScriptInterop` depends on `GeometryScripting`, so the required Geometry Script plugin path is already covered.
- Existing local precedent: `SourceArt/GeometryScript/create_rainy_convenience_street.py` already creates `unreal.DynamicMesh`, appends Geometry Script primitives, recomputes normals, and uses `GeometryScript_NewAssetUtils.create_new_static_mesh_asset_from_mesh` to bake Static Mesh assets. Existing PCG Python authoring helpers in `Plugins/CustomTools/Content/Python/ArtScripts/CubelessRoadPCG.py` create/update PCG graphs and native PCG nodes.
- Sibling MCP status: `D:/Git/unreal-mcp-cubeless` exists and already exposes PCG/editor helper tools such as `refresh_pcg_components`, `open_editor_level`, `capture_viewport_bookmark_screenshot`, PCG graph node helpers, and save/compile helpers. No new MCP C++ command is needed before the dungeon MVP.
...
## 2026-06-17 BP Authoring Completion Branch Sections 313-320

- Executed the post-recreation temp Actor Blueprint reauthoring checkpoint for `/Game/_MCP_Temp/DurableSaveGate/BP_DurableSaveGatePrep`; this opened only `_MCP_Temp` variable, component, Actor tag/default, compile, save, and readback.
- The live UnrealMCP bridge was attached to `D:/Git/CubelessStylized-delete-sky-main/StylizedCubeless.uproject`, so no write was sent through that wrong-project bridge. Correct-project write/readback ran through headless UE `D:/Git/CubelessStylized/StylizedCubeless.uproject`.
- Saved `Content/_MCP_Temp/DurableSaveGate/BP_DurableSaveGatePrep.uasset` with variable `MCPAuthoringScalar=1.0`, `SceneComponent` `MCPAuthoringProbeComponent`, and CDO tag `MCP_DurableAuthoring_LiveProbe`; final readback confirmed the target package was clean after save and file size became `26627` bytes.
- Delete, rename, overwrite, cleanup, graph repair, node/pin mutation, and production path writes stayed closed.
