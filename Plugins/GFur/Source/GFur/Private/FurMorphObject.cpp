...
void FFurMorphObject::Update_RenderThread(FRHICommandListImmediate& RHICmdList, FMorphTargetWeightMap& ActiveMorphTargets, const TArray<float>& MorphTargetWeights, const TArray<TArray<int32>>& InMorphRemapTables, int InMeshLod)
{
	...
	if (NumVertices > 0)
	{
		...
		for (const auto& pair : ActiveMorphTargets)
		{
			...
			checkSlow(ActiveMorphTarget->HasDataForLOD(InMeshLod));
			...
			for (FMorphTargetDeltaIterator DeltaIt = ActiveMorphTarget->GetDeltaIteratorForLOD(InMeshLod); !DeltaIt.AtEnd(); ++DeltaIt)
			{
				const FMorphTargetDelta& MorphVertex = *DeltaIt;
