...
public:
	...
	const TArray<FSection>& GetSections_RenderThread() const { check(IsInRenderingThread() || IsInParallelRenderingThread()); return Sections; }
	int32 GetNumVertices_RenderThread() const { check(IsInRenderingThread() || IsInParallelRenderingThread()); return VertexCount; }
	const FIndexBuffer* GetIndexBuffer_RenderThread() const { check(IsInRenderingThread() || IsInParallelRenderingThread()); return &IndexBuffer; }
