...
FBoxSphereBounds UGFurComponent::CalcBounds(const FTransform& LocalToWorld) const
{
	if (SkeletalGrowMesh)
	{
		...
		{
			return MasterPoseComponent->CalcBounds(LocalToWorld).ExpandBy(FMath::Max(FurLength, 0.001f));
		}
		return SkeletalGrowMesh->GetBounds().ExpandBy(FMath::Max(FurLength, 0.001f)).TransformBy(LocalToWorld);
	}
	else if (StaticGrowMesh)
	{
		return StaticGrowMesh->GetBounds().ExpandBy(FMath::Max(FurLength, 0.001f)).TransformBy(LocalToWorld);
	}
	return FBoxSphereBounds(FVector(0, 0, 0), FVector(0, 0, 0), 0).ExpandBy(FMath::Max(FurLength, 0.001f)).TransformBy(LocalToWorld);
}
